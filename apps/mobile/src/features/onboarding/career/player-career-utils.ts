...
export function getEntrySeasonLabels(entry: PlayerCareerEntry): string[] {
  if (entry.type === "CUSTOM_PERIOD" && entry.period) {
    return computePlayerSeasonsFromPeriod(entry.period);
  }

  return entry.seasons;
}

export function getOccupiedPlayerSeasonLabels(
...
export function getPlayerSeasonSelectOptions(
  occupiedSeasons: Set<string>,
): SelectOption[] {
  return getPlayerSeasonOptions().map((season) => ({
...
export function getOlderPlayerSeasonSelectOptions(
  occupiedSeasons: Set<string>,
): SelectOption[] {
  return getOlderPlayerSeasonOptions().map((option) => ({
    ...option,
...
export function getPlayerEndYearOptions(
  startYear: string,
  startMonth: string,
  occupiedSeasons: Set<string>,
): SelectOption[] {
...
export function sanitizePlayerPeriodSelection(
  period: NonNullable<PlayerCareerEntry["period"]>,
  occupiedSeasons: Set<string>,
): NonNullable<PlayerCareerEntry["period"]> {
  let nextPeriod = { ...period };

  if (nextPeriod.startYear && nextPeriod.endYear) {
    const endYearOptions = getPlayerEndYearOptions(
