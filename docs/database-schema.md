...
- `20260722100000_search_cerca.sql` introduce le RPC di "Cerca": `search_global`, `search_profiles_page`, `search_clubs_page`, `search_positions_page` (tutte security definer, autenticazione richiesta, sole authenticated in grant).
- `20260724100000_search_profiles_filters.sql` (CER-02) ridefinisce `search_profiles_page(p_query, p_role, p_limit, p_offset, p_filters jsonb, p_sort text)`: mantiene le 11 colonne originali in testa e aggiunge metadati per ruolo (`birth_year`, `current_category`, `coach_top_license`, `coach_context`, `agent_operating_areas`, `years_experience`, ...) piu' `total_count bigint` (`count(*) over()`). `p_filters` accetta uno schema jsonb con chiavi condivise (`region`, `is_available`) e gruppi opzionali `player`/`coach`/`staff`/`agent`; ogni predicato per ruolo e' guardato da `p_role is distinct from '<ruolo>'` cosi' da restare inerte sul chip "Tutti". `p_sort` accetta `relevance | recent | vicini | classe_asc | classe_desc` (altrimenti solleva errore). Filtri su province/stagioni/anni esperienza sono proxy best-effort documentati nell'header della migrazione; indici GIN sugli array sono deliberatamente rimandati.
